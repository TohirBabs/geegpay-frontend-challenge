...
const Sidebar = ({navOpen, darkMode, setDarkMode}) => {
  const navItems = [{ title: "category", icon: <Category /> },
  ...
  return (
    <div
      ...
      className='flex transition-all z-10 flex-col h-screen md:p-3 p-2 gap-6 bg-[#f7f8fa] pb-8 md:sticky absolute md:left-0 transition-all  top-0  border md:border-none'>        
          <img src='/logo.svg' className='w-10' alt='site logo' />         
          <div className='flex flex-col  flex-1 items-center justify-between'>    
            <ul className="flex flex-col items-center gap-1 ">
            {navItems.map((nav, index) => index < 7 && (
            <li key={index}>                       
          ...
          <div className='p-3 gap-6 flex flex-col items-center relative rounded-full bg-[#fff]'>
            ...
            }} className='w-8 h-8 bg-[#34caa5] rounded-full absolute transition-all'></div>
